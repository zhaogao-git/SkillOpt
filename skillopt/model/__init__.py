...
from skillopt.model import copilot_cli_backend as _copilot_cli
...
def set_backend(name: str | None) -> str:
    ...
    if normalized in {"copilot", "copilot_cli", "copilot-cli"}:
        set_optimizer_backend("copilot_cli")
        set_target_backend("copilot_cli")
        return "copilot_cli"
...
def get_backend_name() -> str:
    ...
    if optimizer == "openai_compatible" and target == "openai_compatible":
        return "openai_compatible"
    if optimizer == "copilot_cli" and target == "copilot_cli":
        return "copilot_cli"
...
def chat_optimizer(
    ...
    if get_optimizer_backend() == "copilot_cli":
        return _copilot_cli.chat_optimizer(
            system=system,
            user=user,
            max_completion_tokens=max_completion_tokens,
            retries=retries,
...
def chat_target(
    ...
    if get_target_backend() == "copilot_cli":
        return _copilot_cli.chat_target(
            system=system,
            user=user,
            max_completion_tokens=max_completion_tokens,
            retries=retries,
    ...
    if not is_target_chat_backend():
        raise NotImplementedError(
            ...
            "openai_compatible, or copilot_cli. Exec backends are handled in environment-specific rollout code."
        )
...
def chat_optimizer_messages(
    ...
    *,
    ...
    if get_optimizer_backend() == "copilot_cli":
        return _copilot_cli.chat_optimizer_messages(
            messages=messages,
            max_completion_tokens=max_completion_tokens,
            retries=retries,
            stage=stage,
...
def chat_target_messages(
    ...
    *,
    ...
    if get_target_backend() == "copilot_cli":
        return _copilot_cli.chat_target_messages(
            messages=messages,
            max_completion_tokens=max_completion_tokens,
            retries=retries,
            stage=stage,
    ...
    if not is_target_chat_backend():
        raise NotImplementedError(
            ...
            "minimax_chat, openai_compatible, or copilot_cli. Exec backends are handled in environment-specific "
            "rollout code."
        )
...
def get_token_summary() -> dict:
    ...
    for stage, values in codex_summary.items():
        ...
        summary[stage]["completion_tokens"] += values["completion_tokens"]
        summary[stage]["total_tokens"] += values["total_tokens"]
    copilot_cli_summary = _copilot_cli.get_token_summary()
    for stage, values in copilot_cli_summary.items():
        if stage == "_total":
            continue
        if stage not in summary:
            summary[stage] = values
...
def reset_token_tracker() -> None:
    ...
    _copilot_cli.reset_token_tracker()
...
def set_reasoning_effort(effort: str | None) -> None:
    ...
    _copilot_cli.set_reasoning_effort(effort)
...
def set_target_deployment(deployment: str) -> None:
    ...
    _copilot_cli.set_target_deployment(deployment)
...
def set_optimizer_deployment(deployment: str) -> None:
    ...
    _copilot_cli.set_optimizer_deployment(deployment)
